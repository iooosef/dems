<template>
  <PrimeDialog
    v-model:visible="displayNewEntryForms.evacuees" :modal="true"
  ...
  >
    <template #header>
      <div class="flex-1 flex flex-row justify-between text-black">
        ...
        <button class="p-1 text-dark-500 rounded-lg hover:bg-light-500 focus:shadow-inner focus:outline-none focus:ring focus:ring-offset-0 focus:ring-light-500/50 transition duration-200" @click="closeFormModal">
...
<script>
import { } from 'vue'
import { displayNewEntry, displayNewEntryForms, closeFormModal } from '@/composables/dialogModal'
...
export default {
  ...
  setup (props) {
    return { displayNewEntry, displayNewEntryForms, closeFormModal }
  },
  watch: {
  }
}
